...
<?php  //INSERT INTO `invoice`(`serial`, `invoice`, `user_id`, `name`, `price`, `qty`, `tprice`, `point`, `tpoint`, `sdate`, `type`, `ack`)
				$n=1;
				$query=$mysqli->query("SELECT * FROM `invoice` WHERE `user_id`='$id' and `type`='1' order by serial desc");
			...
			?>
   <tr>
        <th class="center"  scope="row">Invoice</th>
		...
		<?php }else{ ?>
		<?php if($inv->paid==0){ ?> 
		<a href="report_my_invoice_act.php?invoice=<?php echo $inv->invoice; ?>&&chk=1"><span class="label label-danger">Shop Delivery</span></a>
		<?php }else{	?>
		...
		<?php }	?>
		 ...
		 <td class="center">
		<a target="_blank" href="report_my_invoice_print.php?invoice=<?php echo $inv->invoice;?>&&userid=<?php echo $inv->user_id;?>">
		<span class="label label-warning">Viewi<span>
		</a>
		</td>
    </tr>
