<?php ob_start();
	...
	if($acc=="dealer"){	
		...
		if($chk_dealer==1){	
			if(($_SESSION['last_trx_time3']!=time())&&($memId!=$receiver)&&($valid==1)){
...
Date:$time $date";
/* Shopping Balance Transaction
Form:$sender_member_profile->user_id,
...
if($acc=="admin"){
		...
		if($chk_admin==1){	
		if(($_SESSION['last_trx_time2']!=time())&&($valid==1)&&($memId!=$receiver)){
		$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','3','0','1','3','1')");
...
$txt ="
Shopping Balance Transaction
Form:$sender_member_profile->log-id,
MemberID:$sender->log_id,
To:Admin,
AdminID:$receiver->user,
Tax:$taxamn,
Amount:$amount
Total:$totalamount,
Date:$time $date";
...
if($check_mem==1){
	if($memId!=$receiver){
		...
		if(($_SESSION['last_trx_time']!=time())&&($valid==1)&&($trxId!='admin')){
		$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','3','0','1','3','1')");
