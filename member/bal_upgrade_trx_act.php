<?php ob_start();
	...
	$memId=$_SESSION['MemLogId']; 
		...
		$location="bal_upgrade_trx.php?page=Transfer Upgrade Balance&&menu=Balance";
		...
		$bal=mysqli_fetch_object($mysqli->query("SELECT `stepup` FROM `balance` WHERE `user_id`='".$memId."'"));
		$pinchk=$sender->pin;
		$net=$bal->stepup;
		...
		if($acc==''){ $valid=0;
			...
			if($sender->log_id==$trxId){  //Up to Cash
	...
	if($acc=="dealer"){	
		...
		if($chk_dealer==1){	
			if(($_SESSION['last_trx_time3']!=time())&&($valid==1)&&($amount>=$installmentwallet)&&($pinchk==$pin)&&($memId!=$receiver)&&($trxId!='')&&($pin!='')&&($amount!='')&&($totalamount<=$net)){
			$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
			VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','4','0','1','3','1')");
...
Total:$totalamount,	
Date:$time $date";
			$headers = "From: Upgrade Balance Transaction<$email>". "\r\n" . "BCC:$receiver_dealer_profile->email";
...
require('../db/api_sms.php');
		...
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','4','0','1','3','1')");
