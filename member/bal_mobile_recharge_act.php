<?php ob_start();
	...
	$memId=$_SESSION['MemLogId']; 
		...
		$location="bal_mobile_recharge.php?page=Mobile%20Recharge&&Payment_Mathod=recharge&&menu=Balance";
	...
	if(($valid==1)&&($mobile!='')&&($amount>0)){		
			...
			if($rsl_api != null){
				...
				if(trim($obj->{'status'}) == "Success"){					
					...
					header("Location:bal_mobile_recharge.php?page=Mobile%20Recharge&&Payment_Mathod=recharge&&menu=Balance");
					exit();
				}else{
					$result=$obj->{'reason'};
