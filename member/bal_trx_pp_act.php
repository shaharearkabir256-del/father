<?php ob_start();
	...
	$memId=$_SESSION['MemLogId']; 
		...
		$location="bal_trx_pp.php?page=Transfer%20PP%20Balance&&menu=Balance";
		...
		$bal=mysqli_fetch_object($mysqli->query("SELECT `balance_purchase_point` FROM `balance` WHERE `user_id`='".$memId."'"));
		$pinchk=$sender->pin;
		$net=$bal->balance_purchase_point;
		...
		if($acc==''){ $valid=0;
			...
			if($amount<$installmentwallet){ $valid=0;
	...
	if($acc=="dealer"){	
		...
		if($chk_dealer==1){	
			if(($_SESSION['last_trx_time3']!=time())&&($valid==1)&&($pinchk==$pin)&&($memId!=$receiver)&&($trxId!='')&&($pin!='')&&($amount!='')&&($totalamount<=$net)){
			$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
			VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','5','0','1','3','1')");
			
/* $to ="<$sender_member_profile->email>"; 
$subject="$sender_member_profile->fname";
$txt = "
Shopping Balance Transaction 
Form:$sender_member_profile->fname, 
MemberID:$sender->log_id, 
To:$receiver_dealer_profile->fname, 
AgentID:$receiver->log_id, 
Tax:$taxamn, 
Amount:$amount, 
Total:$totalamount, 
Date:$day $time $date";
...
require('../db/api_sms.php');
			...
			$_SESSION['msg']= "Transaction Failed 401";
			header("Location:$location");
			exit();		
			}
		}
...
/* if($acc=="admin"){
		...
		if($chk_admin==1){	
		if(($_SESSION['last_trx_time2']!=time())&&($valid==1)&&($amount>=$installmentwallet)&&($pinchk==$pin)&&($memId!=$receiver)&&($trxId!='')&&($pin!='')&&($amount!='')&&($totalamount<=$net)){
		$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','5','0','1','3','1')");
...
require('../db/api_sms.php'); *//*
		...
		$_SESSION['msg']= "Transaction Failed 402";
		header("Location:$location");
		exit();		
		}
	}
...
if($check_mem==1){
	if($memId!=$receiver){
		...
		if(($_SESSION['last_trx_time']!=time())&&($valid==1)&&($amount>=$installmentwallet)&&($pinchk==$pin)&&($trxId!='admin')&&($trxId!='')&&($pin!='')&&($amount!='')&&($totalamount<=$net)){
		$mysqli->query("INSERT INTO `trx`(`trx_id`,`send_id`,`amount`,`tax`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','".$receiver."','5','0','1','3','1')");
...
require('../db/api_sms.php');
		...
		$_SESSION['msg']= "Transaction Failed 403";
		header("Location:$location");
		exit();		
		}
	}else{
		...
		VALUES('".$trx_id."','".$memId."','".$amount."','".$taxamn."','".$date."','".$time."','".$day."','5','0','1','3','1')");
		$mysqli->query("INSERT INTO `trx`(`trx_id`,`amount`,`date`,`time`,`day`,`rec_id`,`type`,`method`,`status`,`account`,`take`) 
		VALUES('".$trx_id."','".$amount."','".$date."','".$time."','".$day."','".$receiver."','4','0','1','3','1')");
			
$to ="<$sender_member_profile->email>";
 $email=$sender_member_profile->email;
$subject="$sender_member_profile->fname";
$message ="
Purchase Point Transaction 
Form:$sender->log_id (PP) 
To:upgrade Wallet, 
Amount:$amount,
Tax:$taxamn,
Total:$totalamount, 
...
/* $headers = "From: Purchase Point Transaction<$email>". "\r\n" . "BCC:$receiver_member_profile->email";
mail($to,$subject,$txt,$headers); */
...
require('../db/api_sms.php');
		...
		$_SESSION['msg']= "Transaction Failed 404";
		header("Location:$location");
		exit();		
		}
	}
