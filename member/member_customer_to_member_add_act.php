<?php ob_start();
	...
	$location="member_customer_to_member_add.php?page=Member Create From Purchase Point";
    if($agent==''){
...
// Sponsor Id
		...
		$member=mysqli_fetch_object($mysqli->query("select * from `member` where `user_id`='".$id."' "));
		$spbal=$member->point; // Customer Temp Point Field
		if($spbal<$plan){		
			$_SESSION['msg']="Need Registration Fee $plan Point";		
			header("Location:$location");		
			exit();		
		}else{ $cus_pp=($spbal-$plan); } 		
...
// Commission Distribute Sponsor And Submition Limit Check
 ...
 // Happy mem join lim 2 
	...
	if($chk_upline==1){$placecode=6;}
		...
		if($totlamembers>=$setting->mem_join_lim){$_SESSION['msg']="Can't Submit More Than Maximum Submittion";header("Location:$location");exit();}
	...
	if(($chkplace==0)&&($chk_upline<2)&&($agent!='')&&($spbal>=$plan)&&($chk_sponsor==1)&&($totlamembers<$setting->mem_join_lim)&&($tree2->active==1)&&($check==1)){
		...
		//gen crn_gen
    	$mysqli->query("UPDATE `member` SET `team`=0,`stype`='$stype',`sponsor`='".$spot_ref."',`position`='".$placecode."',`point`='".$plan."',`upline`='".$uplineUser."',`agent_id`='".$agent."', `date`='".$date."' WHERE `user_id`='".$id."' "); 
    	$logId=$member->user_id; 
    	$logIdP=$member->log_id; 
		//match || crn_daily || crn_match
		$mysqli->query("INSERT INTO `tree`(`plan`,`team`,`point`,`stype`,`user_id`,`user`,`position`,`upline`,`sponsor`,`agent_id`,`date`) 
		VALUES ('$plnsn','1','".$plan."','$stype','".$id."','".$member->log_id."','".$placecode."','".$uplineUser."','".$spot_ref."','".$agent."','".$date."')");
		$mysqli->query("UPDATE `profile` SET stype='$stype',`nid`='$nid' WHERE `user_id`='".$id."' ");
		$mysqli->query("INSERT INTO `balance` (`user_id`) VALUES ('".$id."')");
		// sponsor com | Agent Com | Daily Payment from here
		$mysqli->query("INSERT INTO `invest`(`team`,`fund_donation`,`fund_company`,`trx_id`, `agent_id`, `agent_com_percent`, `agent_com`, `uw_id`, `uw_com_percent`, `uw_com`, `upazila_id`, `upazila_com_percent`, `upazila_com`, `district_id`, `district_com_percent`, `district_com`, `zone_id`, `zone_com_percent`, `zone_com`, `user_id`, `invest`, `invest_id`, `payable`, `sponsor`, `stepup`, `upline`, `shopping`, `date`, `club`)
		VALUES ('1','".$fund_donation."','".$fund_company."','".$trx_id."','$agent','$setting->mem_join_agent_com','".$agent_com."','$uw_id','$setting->mem_join_uw_com','".$uw_com."','$upazila_id','$setting->mem_join_upazila_com','".$upazila_com."','$district_id','$setting->mem_join_district_com','".$district_com."','$zone_id','$setting->mem_join_zone_com','".$zone_com."','".$id."','".$plan."','$id','".$spCom."','".$spot_ref."','".$stCom."','".$uplineUserId."','".$shopping."','".$date."','$placecode')");
		...
		$mysqli->query("update `balance` set `balance_purchase_point`='".$cus_pp."' where `user_id`='$id' ");
...
$headers = "From:Registration By Member Using PP:$totlamembers<$email>". "\r\n" . "BCC:$gmail";
...
include'../phpmailer/send_mail.php'; 
		...
		$sms="DIBOSAH: মেম্বার হিসেবে স্বাগতম। Login:$url/member";
					
		require('../db/api_sms.php');

		$_SESSION['msgs']="Registration Successful";
		header("Location:member_purchase_add.php"); 
		exit();
	}else{
		$_SESSION['msg'] = "Failed";
		header("Location:$location");
