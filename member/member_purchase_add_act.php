<?php ob_start();
	...
	$location="member_purchase_add.php";
...
// Sponsor Id
		...
		$check=mysqli_num_rows($q2);
			...
			$bal=mysqli_fetch_object($mysqli->query("select `balance_purchase_point` from `balance` where `user_id`='$id' "));
			$spbal=($bal->balance_purchase_point);	
...
// Commission Distribute Sponsor And Submition Limit Check
   ...
   // Happy mem join lim 2 
	...
	if(($chkplace==0)&&($chk_upline<2)&&($agent!='')&&($spbal>=$plan)&&($chk_sponsor==1)&&($totlamembers<$setting->mem_join_lim)&&($tree2->active==1)&&($chkm==0)&&($check==1)){
		...
		VALUES('$stype','".$logId."','".$logIdP."','".$dpass."','".$dpin."','".$spot_ref."','".$placecode."','".$plan."','".$uplineUser."','".$agent."','".$date."')");
		...
		VALUES ('$plnsn','2','$plan','$stype','$logId','".$logIdP."','".$placecode."','".$uplineUser."','".$spot_ref."','".$agent."','".$date."')");
