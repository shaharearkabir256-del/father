<?php ob_start();
	...
	$memef=$mysqli->query("select * from `tree` where `user`='".$sponsor."' and `stype`='$stype1' "); 
	...
	if($agent==''){
		...
		// Placement Id
	if($uplink!=''){		
		$exeupline=$mysqli->query("select * from `tree` where `user`='".$uplink."' and `stype`='$stype1' ");
...
// User Id
	...
	if($chk_sponsor==0){
		$_SESSION['msg']="Invalid Sponsor ID";
		header("Location:$location");
		exit();
	}
	if($nid==''){
...
// Sponsor Id
		...
		$check=mysqli_num_rows($q2);
			...
			if($spbal<$point){		
				$_SESSION['msg']="Need Registration Fee $point Point";		
				header("Location:$location");		
				exit();		
			} 		
...
// Commission Distribute Sponsor And Submition Limit Check
   ...
   // Happy mem join lim 2 
	...
	header("Location:$location"); 
	exit();	
	}else{ $spCom=$point*$setting->mem_join_spot_com2/100;
	...
	exit(); 
		...
		$spotcom=$point*$setting->mem_join_spot_com/100;
	...
	exit(); 
	    ...
	    $spotcom=$point*$setting->mem_join_spot_com3/100;
    	$spCom=$spotcom*$setting->mem_join_spot_cash_wallet/100;
    	$stCom=$spotcom*$setting->mem_join_spot_upgrade_wallet/100;
    	$shopping=$spotcom*$setting->mem_join_spot_shopping_wallet/100;
	...
	//Freedom mem join lim 2
	if($stype==4){ 
	$chk_upline=mysqli_num_rows($mysqli->query("select * from `tree` where `stype`=4 AND `upline`='".$uplineUser."' "));
	if($chk_upline>=2){	
	$_SESSION['msg']="Freedom Submittion Complete";
	header("Location:$location"); 
	exit(); 
	}else{ 
	    $spotcom=$point*$setting->mem_join_spot_com3/100;
	...
	if(($chkplace==0)&&($chk_upline<2)&&($agent!='')&&($spbal>=$point)&&($chk_sponsor==1)&&($totlamembers<$setting->mem_join_lim)&&($tree2->active==1)&&($chkm==0)&&($check==1)){
		$logId=time();	
		
		$del=mysqli_fetch_object($mysqli->query("SELECT * FROM `dealer` WHERE `user_id`='".$agent."'"));
		...
		$agent_com=($point*$setting->mem_join_agent_com/100);
		$uw_id=$war->user_id; 
		$uw_com=($point*$setting->mem_join_uw_com/100);
		$upazila_id=$uni->user_id;
		$upazila_com=($point*$setting->mem_join_upazila_com/100);
		$district_id=$upz->user_id;
		$district_com=($point*$setting->mem_join_district_com/100);
		$zone_id=$zon->user_id;
		$zone_com=($point*$setting->mem_join_zone_com/100); 
		$fund_donation=($point*$setting->mem_join_donation_fund/100); 
		$fund_company=($point*$setting->mem_join_company_fund/100);
		//gen crn_gen
		$mysqli->query("INSERT INTO `member`(`stype`,`user_id`,`log_id`,`pass`,`pin`,`sponsor`,`position`,`point`,`upline`,`agent_id`,`date`) 
		VALUES('$stype','".$logId."','".$logIdP."','".$dpass."','".$dpin."','".$spot_ref."','".$placecode."','".$point."','".$uplineUser."','".$agent."','".$date."')");
		...
		VALUES ('$plan','".$point."','$stype','".$logId."','".$logIdP."','".$placecode."','".$uplineUser."','".$spot_ref."','".$agent."','".$date."')");
		...
		VALUES ('".$fund_donation."','".$fund_company."','".$trx_id."','$agent','$setting->mem_join_agent_com','".$agent_com."','$uw_id','$setting->mem_join_uw_com','".$uw_com."','$upazila_id','$setting->mem_join_upazila_com','".$upazila_com."','$district_id','$setting->mem_join_district_com','".$district_com."','$zone_id','$setting->mem_join_zone_com','".$zone_com."','".$logId."','".$point."','$id','".$spCom."','".$spot_ref."','".$stCom."','".$uplineUserId."','".$shopping."','".$date."','$placecode')");
...
$headers = "From:Registration By Member:$totlamembers<$email>". "\r\n" . "BCC:$gmail";
...
include'../phpmailer/send_mail.php'; 	
		...
		//$mobile="88$mobile";
		$sms="DIBOSAH: মেম্বার হিসেবে স্বাগতম। UserID:$logIdP Password:$pass Pin:$dpin https://$url/member";			
