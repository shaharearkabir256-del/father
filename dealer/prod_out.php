  ...
  <body class="skin-blue">
	...
	<tbody>
			<?php $n=1;
				...
				$query=$mysqli->query("SELECT * FROM `stock` where `send_id`='$myid' order by serial desc ");
    ...
    <tr <?php if($date==$product->date){ ?> bgcolor="#e6f2ff" <?php } ?> >
		...
		<td><?php echo $product->price.$tk; ?></td> 	<td><?php  echo $product->rp; ?></td>
		
							
		<td><?php
		$del=mysqli_fetch_object($mysqli->query("SELECT * FROM `dealer` WHERE `user_id`='$product->rec_id' "));
		echo $del->log_id; ?></td>
