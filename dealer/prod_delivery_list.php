          ...
          <div class="row">
            <div class="col-xs-12">
              ...
              <div class="box box-primary">
				...
				<?php if($_SESSION['DealerLogId']!=''){?>
				 <table id="example1" class="table table-bordered table-hover">
                    <thead>
                      <tr>
        <th>#</th>
			<th>Sales To UserId</th>
        <th>Pic</th>
        <th>Product Name</th>
		<th>Price</th>	
		<th>Point</th>
...
$totalsal=mysqli_fetch_object($mysqli->query("SELECT sum(price)as tp, sum(rp)as tr, sum(qty)as tq FROM `sales` where `seller_id`='$myid' and `chk`='1'"));

				$query=$mysqli->query("SELECT * FROM `sales` where `seller_id`='$myid' and `chk`='1' order by serial desc ");
	...
	<tbody>
			<?php 
				...
				$query=$mysqli->query("SELECT * FROM `order` where `agent_id`='".$myid."' and chk=0 order by serial desc ");
    ...
    <tr <?php if($date==$product->date){ ?> bgcolor="#e6f2ff" <?php } ?> >
		...
		<td><?php 
			$q2=$mysqli->query("select * from `product` where `serial`='".$product->product_id."' ");
