...
if( $_SESSION['DealerLogId'] == ''){ 
		...
		$location='prod_order_list.php?pageName=Product%20Order%20List';
		...
		$bal=mysqli_fetch_object($mysqli->query("SELECT * FROM `balance` where `user_id`='".$res1->send_id."'"));
/* 		if($bal->net_bal<$cash){
		$_SESSION['msg']="Insufficient Member Balance";       
		header("Location:$location");
		exit();	
		} */
	...
	/* 	if($delrec->type==1){
		...
		//if($bal->net_bal>=$cash){
			if($reqstkchk==0){
				...
				$mysqli->query("update `prod_req` set `total`='$cash',`chk`='1' where `serial`='".$prser."' ");
	...
	/* 			$mysqli->query("INSERT INTO `dealer_trx`(`send_id`, `rec_id`, `amount`,`type`, `method`, `status`, `account`, `day`, `date`, `time`, `take`) 
				...
				$mysqli->query("update `prod_req` set `total`='$reqtotalup',`chk`='1' where `serial`='".$prser."' ");
				$mysqli->query("update `stock` set `qty`='$qtyup' where `p_id`='".$res1->p_id."' and `rec_id`='$res1->rec_id' "); // My Stock Up
/* 				$mysqli->query("INSERT INTO `dealer_trx`(`send_id`, `rec_id`, `amount`,`type`, `method`, `status`, `account`, `day`, `date`, `time`, `take`) 
