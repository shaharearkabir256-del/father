...
$totalsal=mysqli_fetch_object($mysqli->query("SELECT sum(price)as tp, sum(rp)as tr, sum(qty)as tq FROM `sales` where `seller_id`='$myid' and `chk`='1' and `date`='".$date."'"));

				$query=$mysqli->query("SELECT * FROM `sales` where `seller_id`='$myid' and `chk`='1' and `date`='".$date."' order by `date` desc ");
    ...
    <tr <?php if($date==$sal->date){ ?> bgcolor="#e6f2ff" <?php } ?> >
        ...
        <th></th>
        <th></th>
		<th>Total Price: <?php  echo $totalsal->tp; ?></th>	
		<th>Total Point: <?php  echo $totalsal->tr; ?></th>
		<th></th>
        <th>Total Qty: <?php  echo $totalsal->tq; ?></th>
