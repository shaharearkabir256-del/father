<?php 
	session_start();
	if( $_SESSION['DealerLogId'] == ''){ 
	$_SESSION['msg']="Please Verify login!";
	header("Location:logout.php");
	exit();
	}
	else{
		require '../db/db.php';
		$memId=$_SESSION['DealerLogId'];
			
		$dpass=$mysqli->real_escape_string($_POST['password1']);
		$pass=md5($dpass);		
		$epass=$mysqli->real_escape_string($_POST['password2']);
		$cpass=md5($epass);

		$location="profile.php?page=User Profile&active=pin";
		$adm=mysqli_fetch_object($mysqli->query("SELECT * FROM `dealer` where `user_id`='$memId'"));
		$pro=mysqli_fetch_object($mysqli->query("SELECT * from `dealer_info` where `user_id`='$memId' "));
		$gmail=$pro->email;	
		$fname=$pro->fname;
		$lname=$pro->lname;
			if($gmail==''){
			$_SESSION['msg'] = "Please Enter <b>E-Mail<b> at <b>Profile Info</b> ";
			header("Location:profile.php?page=User Profile&f=email");
			exit();
			}
			if($dpass==''){
			$_SESSION['pnmsg'] = "Please Enter Password ";
			header("Location:$location");
			exit();
			}
			if($epass==''){
			$_SESSION['pnmsg'] = "Please Enter Confirm Password ";
			header("Location:$location");
			exit();
			}
			if($dpass!=$epass){
			$_SESSION['pnmsg'] = "Not Match ";
			header("Location:$location");
			exit();
			}
		
		if(($gmail!='')&&($dpass==$epass)&&($dpass!='')&&($epass!='')){			
		$mysqli->query("update `dealer` set `pin`='".$epass."', `pndate`='".$date."' where `user_id`='".$memId."'");
		$to = "$fname $lname<$gmail>";
			$subject=$title;
			$txt = "
			Dealer Pin Updated
			
			Your Login Information:
			User Name: $adm->log_id
			Pin: $dpass
			
			Pin Updated Date: $day $time $date
			
			Login Dealer Panel (https://$url/admin)
			";
			
			$headers = "From:Pin Updated<$email>". "\r\n" . "BCC:info@sksitfirm.net";
			mail($to,$subject,$txt,$headers);
$mobile=$pro->mobile;
$sms=$txt;			
require('../db/api_sms.php');
		$_SESSION['pnmsgs']= "Pin Updated";
		header("Location:$location");
		exit();
		}else{
		$_SESSION['pnmsg']= "Failed";
		header("Location:$location");
		exit();
		}
		

	
	}
	
?>