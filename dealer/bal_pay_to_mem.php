...
<?php
		...
		$user=mysqli_fetch_object($q2);
				$n=1;
				$query=$mysqli->query("SELECT * from `withdraw` where `rec_id`='".$accounts."' and type='0' and `account`='3'");
				while($trx=mysqli_fetch_object($query)){
				
			?>
               <?php

		$q2=$mysqli->query("SELECT `log_id` FROM `member` WHERE `user_id`='".$trx->send_id."'");
       ...
       <tr <?php if($chkdel==1){ echo "style='background-color: #ecf4f9;'";} ?>>
        ...
        <?php if($trx->status==0){?>
		<td class="center">
		<a href="bal_pay_y_to_mem.php?sn=<?php echo $trx->serial;?>&sendId=<?php echo $trx->send_id;?>"><button  type="submit" class="btn btn-sm btn-success">Yes</button></a>	
		<a href="bal_pay_n_to_mem.php?sn=<?php echo $trx->serial;?>&sendId=<?php echo $trx->send_id;?>"><button  type="submit" class="btn btn-sm btn-danger">No&nbsp;</button></a>
		</td>
        <?php }else{ ?>
		<td class="center">
